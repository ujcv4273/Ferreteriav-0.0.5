...
def reporte_compras(request):
    template_path = 'gestionadmin/compras_print_all.html'
    today = timezone.now()
...
def reporte_comprasexcel(request):
    ...
    columns = ['ID', 'Numero factura', 'Cliente','Subtotal', 'ISV', 'Total', ]
    for col_num in range(len(columns)):
        ws.write(row_num, col_num, columns[col_num], font_style)
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = ComprasEnc.objects.all().values_list('id','no_factura','cliente','sub_total','descuento','total')
    for row in rows:
        row_num += 1
...
def imprimir_compra(request, compra_id):
    template_path = 'gestionadmin/compras_print_one.html'
    today = timezone.now()
